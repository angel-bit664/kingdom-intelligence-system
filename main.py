...
BANDERAS = {
'🇺🇸': 'en', '🇧🇷': 'pt', '🇫🇷': 'fr', '🇩🇪': 'de', '🇮🇹': 'it', '🇷🇺': 'ru', '🇯🇵': 'ja', '🇰🇷': 'ko', '🇨🇳': 'zh-CN', '🇸🇦': 'ar', '🇹🇷': 'tr', '🇮🇩': 'id', '🇹🇭': 'th', '🇻🇳': 'vi', '🇵🇱': 'pl'
}
NOMBRES_IDIOMAS = {
...
async def on_message(message):
    ...
    if comando == "activate":
        ...
        try:
            ...
            if mensaje_custom:
                ...
                mensaje_extra = f"\n\n💬 MENSAJE / MESSAGE:\n🇲🇽 {datos['es']}\n🇺🇸 {datos['en']}"
            descripcion = f"""🚨 CÓDIGO DE EMERGENCIA TFT 🚨 ⚠️ ALERTA ROJA / RED ALERT ⚠️ 🎯 OBJETIVO / TARGET:{usuarios_texto} ❌ ESTADO / STATUS: {texto_sin} {texto_escudo} ACTIVO - ZONA DE PELIGRO NO ACTIVE SHIELD - DANGER ZONE 🛡️ PROTOCOLO DE EMERGENCIA / EMERGENCY PROTOCOL: 1. {texto_plural} INMEDIATAMENTE / CONNECT NOW 2. ESCUDO 8H YA / 8h SHIELD NOW 3. TELEPORT DE EMERGENCIA / EMERGENCY TELEPORT{mensaje_extra} ⚔️ ALIANZA TFT EN ALERTA MÁXIMA TFT ALLIANCE ON MAXIMUM ALERT Código emitido por: {autor.display_name} ⏰ TIEMPO ES CRÍTICO / TIME IS CRITICAL"""
    ...
    if comando in ["cumpleaños", "cumpleanos"]:
        ...
        descripcion = f"""🎉 FELIZ CUMPLEAÑOS 🎉 🎂 CELEBRACIÓN ESPECIAL / SPECIAL CELEBRATION 🎂 🎯 CUMPLEAÑERO / BIRTHDAY:{usuario_cumple.mention} 🎊 ESTADO / STATUS: DÍA DE FIESTA - MODO CELEBRACIÓN ACTIVADO CELEBRATION MODE - PARTY TIME 🎁 MENSAJE / MESSAGE: 🇲🇽 {datos['es']} 🇺🇸 {datos['en']} ⚔️ LA FAMILIA TFT TE CELEBRA TFT FAMILY CELEBRATES YOU Felicitación enviada por: Todo el grupo de Oficiales 📍 QUE LA PASES INCREÍBLE / HAVE AN AMAZING TIME"""
