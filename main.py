...
# ========== AUTO-PING PARA EVITAR QUE SE DUERMA ==========
async def self_ping_loop():
    await asyncio.sleep(60) # Espera 1 min antes de empezar
    url = f"https://{os.environ.get('RENDER_EXTERNAL_HOSTNAME', 'kingdom-bot-x425.onrender.com')}"
    while True:
        try:
...
async def on_ready():
    ...
    # INICIA EL SELF-PING CUANDO EL BOT PRENDE
    asyncio.create_task(self_ping_loop())

# ========== AUTO-REINICIO SI SE CAE ==========
@client.event
async def on_disconnect():
    print("[ERROR] Bot desconectado de Discord. Reiniciando proceso...")
    os.execv(sys.executable, ['python'] + sys.argv)

@client.event
async def on_error(event, *args, **kwargs):
    print(f"[ERROR] Error en {event}. Reiniciando...")
    os.execv(sys.executable, ['python'] + sys.argv)
# =============================================
